...
// input: filter
// output: array of nodes that pass the filters test
// constraints: call each node in breadth first order
// edge case: if no nodes meet criteria, return empty array ?
Tree.prototype.BFSelect = function (filter, depth) {
  depth = depth || 0
  // base case: if no children
  // return that node run through the filter
  var filteredRoot;
  // run root through filter
  if (filter(this.value, depth)) {
    filteredRoot = [this.value]
  } else {
    filteredRoot = []
  }

  // map or loop through children nodes

  for (var i = 0; i < this.children.length; i++) {
    var child = this.children[i]
    // for each
    // call filter on it
    filteredRoot = filteredRoot.concat(child.BFSelect(filter, depth + 1))
  }
